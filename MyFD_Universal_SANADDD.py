...
with col1:
    if st.button("🔒 Fingerprint / بصمة"):
        st.markdown(f"<div style='background-color:rgba(255,215,0,0.2); padding:10px; border-radius:10px; text-align:center;'>{messages[lang_choice]['fingerprint']}</div>", unsafe_allow_html=True)
    if st.button("🙂 Face / وجه"):
        st.markdown(f"<div style='background-color:rgba(255,215,0,0.2); padding:10px; border-radius:10px; text-align:center;'>{messages[lang_choice]['face']}</div>", unsafe_allow_html=True)
with col2:
    if st.button("🤟 Gesture / إشارة"):
        st.markdown(f"<div style='background-color:rgba(255,215,0,0.2); padding:10px; border-radius:10px; text-align:center;'>{messages[lang_choice]['gesture']}</div>", unsafe_allow_html=True)
    if st.button("🔢 Code / كود"):
        st.markdown(f"<div style='background-color:rgba(255,215,0,0.2); padding:10px; border-radius:10px; text-align:center;'>{messages[lang_choice]['keypad']}</div>", unsafe_allow_html=True)
